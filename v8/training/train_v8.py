"""
v8 stage-1 training: phoneme-level articulatory regression.

Loss = MSE(start) + MSE(end) + MSE(log-duration), masked to valid phoneme positions.
Body-only (BOS/EOS positions excluded from anchor losses but supervised on duration=1).

Usage:
  KMP_DUPLICATE_LIB_OK=TRUE python -u v8/training/train_v8.py --device mps --epochs 10
"""
import argparse
import json
import sys
...
# Allow importing v8 modules without installing as a package
...
def train(args):
    ...
    print(f"v8 phoneme TTS — training on {device}")
    ...
    # ─── Model ───────────────────────────────────────────
    # Vocab size from phoneme_ids range (we assume it matches PhonemeVocab default).
    # Pull from phonemes_path; if not, default 73.
    ...
    if args.resume and (ckpt_dir / "best.pt").exists():
        ...
        if args.restore_optim and "optim" in c:
    ...
    def compute_losses(batch):
        ...
        pred_start, pred_mid, pred_end, pred_log_dur = model.predict(
            phoneme_ids, spk_emb, phoneme_mask, style_emb=None,
        )

        # Body mask: exclude BOS (first valid) + EOS (last valid). Anchor losses on body only.
...
if __name__ == "__main__":
    ...
    p.add_argument("--checkpoint-dir",  default="v8/checkpoints/stage1")
