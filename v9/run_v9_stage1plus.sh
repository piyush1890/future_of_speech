#!/bin/zsh
# v9 stage1+ orchestrator (architectural fix: end_heads condition on start[i]):
#   1. Train Stage 1 (renderer with new start→end conditioning).
#   2. Extract per-phoneme style codes.
#   3. Train Stage 2 (planner).
#   4. Render audio.
#
# Tokenizer + RVQ tokens are kept from previous run (no retrain needed).
...
# Wipe old stage 1 + style codes + stage 2 (architecture changed → incompatible)
rm -rf v9/checkpoints/stage1_renderer
rm -rf v9/checkpoints/stage2_planner
rm -rf v9/data/style_codes

# ─── Step 1: Stage 1 (renderer + style enc + codebook) ──────────────────
