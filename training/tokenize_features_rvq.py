"""
Tokenize all features using trained Residual VQ model.
Saves per-utterance .npy files with shape (T, num_quantizers).
"""
import argparse
import sys
...
def tokenize_all(args):
    ...
    model = ArticulatoryRVQTokenizer(
        ...
        num_quantizers=model_args["num_quantizers"],
        latent_dim=model_args["latent_dim"],
        hidden_dim=model_args["hidden_dim"],
    ).to(device)
    model.load_state_dict(ckpt["model_state_dict"])
    model.eval()
    print(f"Loaded RVQ ({model_args['num_quantizers']} codebooks) from {args.checkpoint}")
    ...
    print(f"Token shape: (T, {model_args['num_quantizers']})")
