"""
Train the Residual VQ tokenizer on SPARC articulatory features.
"""
import argparse
import sys
...
def train(args):
    ...
    print(f"Training Residual VQ on {device}")
    ...
    model = ArticulatoryRVQTokenizer(
    ...
    print(f"Using {args.num_quantizers} codebooks of {args.codebook_size} entries each")

    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)

    # Weight pitch and loudness higher
    channel_weights = torch.ones(14, device=device)
    channel_weights[12] = 2.0
    channel_weights[13] = 2.0
